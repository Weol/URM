...
function urm.loadouts.add(usergroups,strings,primary,secondary,silent)
	...
	if (#usergroups > 0) and (#strings > 0) then
		TIIP.URM.SendCommand("ulx addloadout",string.Implode(",",usergroups),string.Implode(",",strings),primary,secondary,silent)
	end
end
...
function urm.loadouts.delete(usergroups,strings,silent)
	...
	if (#usergroups > 0) and (#strings > 0) then
		TIIP.URM.SendCommand("ulx removeloadout",string.Implode(",",usergroups),string.Implode(",",strings),silent)
	end
end
...
urm.loadouts.primarybutton.DoClick = function()
	...
	TIIP.URM.SendCommand("ulx setprimary",string.Implode(",",usergroups),urm.loadouts.itemlist:GetSelected()[1]:GetValue(2),1)
end
